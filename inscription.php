...
if(isset($_POST['civilite'], $_POST['pseudo'], $_POST['password'], $_POST['confirm_password'], 
$_POST['email'], $_POST['prenom'], $_POST['nom'], $_POST['adresse'], $_POST['ville'], $_POST['code_postal']))
{
    ...
    // On compte combien de résultats retourne la requete SELECT, si elle retourne 1 résultat, cela veut dire que le pseudo saisi dans le 
    //formulaire existe en BDD, si la requete SELECT ne retourne aucun résultat, cela veut dire pseudo n'existe pas en BDD
    ...
    // Si l'indice 'pdc' n'est paqs definit, cela veut  dire que l'internaute n'a pas coché la case des pdc
if(!isset($_POST['pdc']))
{
    ...
    // 9. Si l'internaute a correctement remplit le formulaire, réaliser le traitement PHP + SQL  permettant d'insérer un nouvel
    // utilisateur dans
    // la BDD à la validation du formulaire (PREPARE + BINDVALUE + INSERT + EXECUTE)
    if(!isset($error))
    {   
        ...
        // eho '<pre>'; preint_r($insertMembre) ; echo '<hr>'

        // On execute autant de fois bindvalue que nous avons de marqueurs déclarés
        // On renseigne chaque  chaque valeur inserrée dans chaque marqueur
        ...
        // On stock un message de validatuion dans le fichier session de l'utilisateur
        $_SESSION['valid_inscription'] = "<p class='col-6 bg-success text-white text-center mx-auto p-3'>
        Félicitation ! vous êtes maintenant inscrit sur le site. vous pouvez à présent vous connecter.</p>";

        /* On redirige l'internaute vers la page connexion.php aprés l'executon de la requete d'insertion, apres l'inscription
        de l'internaute.
        */
        header('location: connexion.php');
    }   
...
?>
    ...
    <form method="post" class="row g-3 mb-5">
        ...
        <div class="col-md-6">
            ...
            <input type="text" class="form-control <?php if(isset($errorPseudo)) echo $border; ?>" id="pseudo" name="pseudo" value="
            <?php if(isset($_POST['pseudo'])) echo $_POST['pseudo']; ?>">
            <!-- Si l'indice 'pseudo' est définit dans $_POST, alors on affiche la valeur saisie pour le pseudo dans l'attribut 
            'value', donc comme valeur par défaut dans le champ -->
        ...
        <div class="col-md-6">
            ...
            <input type="password" class="form-control <?php if(isset($errorPassword)) echo $border; ?>
            " id="confirm_password" name="confirm_password">
        </div>
        <div class="col-12">
            <label for="email" class="form-label">Email</label>

            <input type="text" class="form-control <?php if(isset($errorEmail)) echo $border; ?>
            " id="email" name="email" placeholder="Saisir votre adresse email" value="<?php if(isset($_POST['email'])) echo $_POST['email']; ?>">
